...
def _sqlglot_dialect(sqlalchemy_dialect: str) -> str:
    return _SQLALCHEMY_TO_SQLGLOT_DIALECT.get((sqlalchemy_dialect or "").lower(), "sqlite")


def _validate_ast(statements, dialect: str = "sqlite") -> str:
...
def sanitize_sql(sql_query: str, dialect: str = "sqlite") -> str:
    """
    ...
    Outputs SQL re-serialised in the target dialect (default: sqlite).

    Args:
        sql_query (str): The raw SQL string generated by the LLM.
        dialect (str): sqlglot dialect for parsing and output. Use ``_sqlglot_dialect()``
            to convert an SQLAlchemy ``engine.dialect.name`` to the right value.

    Returns:
        str: The sanitised SQL re-serialised in the target dialect.
    ...
    try:
        statements = sqlglot.parse(sql_query, read=dialect)
        return _validate_ast(statements, dialect=dialect)
